...
let mcp;
try {
    mcp = require('@modelcontextprotocol/sdk');
}
catch {
    mcp = {};
}
...
function searchRag(query, limit = 3) {
...
if (process.argv.includes('--probe')) {
    try {
        const tools = server ? Object.keys(((_a = server.capabilities) === null || _a === void 0 ? void 0 : _a.tools) || {}) : [];
        // eslint-disable-next-line no-console
        ...
        // eslint-disable-next-line no-console
        console.error(JSON.stringify({ mcp: false, error: err === null || err === void 0 ? void 0 : err.message }));
        process.exit(1);
    }
}
if (server)
    server.setRequestHandler('tools/call', async (req) => {
        ...
        if (name === 'debug.run') {
            const body = input || {};
            const errorType = (body.error_type || 'SYNTAX');
            const et = (_a = confidence_scoring_1.ErrorType[errorType]) !== null && _a !== void 0 ? _a : confidence_scoring_1.ErrorType.SYNTAX;
            const chat = getChat(body.sessionId || 'default');
            const result = await debuggerInstance.attemptWithBackoff(et, String(body.message || ''), String(body.patch_code || ''), String(body.original_code || ''), Array.isArray(body.logits) ? body.logits : [], { maxAttempts: Math.max(1, body.maxAttempts || 1), sessionId: body.sessionId, chatAdapter: chat });
        ...
        if (name === 'rag.add') {
            ...
            if (existingIdx >= 0)
                RAG_INDEX[existingIdx] = { id, content, tags };
            else
                RAG_INDEX.push({ id, content, tags });
