...
class ConfidenceComponents {
    constructor(historical_success_rate = 0.5, pattern_similarity = 0.5, code_complexity_penalty = 1.0, test_coverage = 0.5) {
...
class ConfidenceScore {
    constructor(overall_confidence, syntax_confidence, logic_confidence, calibration_method = "temperature_scaling", components = new ConfidenceComponents()) {
...
class UnifiedConfidenceScorer {
    ...
    calculate_confidence(logits, error_type, historical_data = null) {
        ...
        const overall_confidence = this._combine_confidences(syntax_confidence, logic_confidence, components, error_type);
...
class DualCircuitBreaker {
    constructor(syntax_max_attempts = 3, logic_max_attempts = 10, syntax_error_budget = 0.03, // 3%
    logic_error_budget = 0.10 // 10%
        ...
        this.recent_error_densities = []; // errors per 100 lines of code
        this.improvement_window = 3; // Look at last 3 attempts for trends
        // Strategy tracking
        this.current_strategy = "simulation";
        this.confidence_floor_for_promotion = 0.85;
        ...
        this.pausedUntil = null; // epoch ms
    ...
    record_attempt(error_type, success, errorsDetected = 0, errorsResolved = 0, confidence = 0, linesOfCode = 100) {
    ...
    isOscillating() {
        ...
        for (let i = 1; i < this.recent_error_counts.length; i++) {
            if (this.recent_error_counts[i] > this.recent_error_counts[i - 1])
                up = true;
            if (this.recent_error_counts[i] < this.recent_error_counts[i - 1])
                down = true;
        }
    ...
    isErrorDensityImproving() {
        /**
         * Check if error density (errors per 100 lines) is decreasing
         */
        if (this.recent_error_densities.length < 2 || this.recent_error_counts.length < 2)
            return false;
        ...
        if (this.isRegressingAgainstBest())
            return false;
    ...
    isRegressingAgainstBest(margin = 1) {
        if (this.best_error_count_seen === null || this.recent_error_counts.length === 0)
            return false;
        const last = this.recent_error_counts[this.recent_error_counts.length - 1];
    ...
    getRecommendedAction() {
        ...
        if (suggestPause) {
            ...
            return "continue"; // will be overridden to pause_and_backoff by wrapper
        }
        if (shouldContinue) {
            return "continue";
        }
