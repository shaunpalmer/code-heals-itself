...
exports.basicBalanceScan = basicBalanceScan;
function extractFunctionContext(originalCode, patchCode) {
...
function basicBalanceScan(code) {
    const counts = (chars) => (code.match(new RegExp(chars, 'g')) || []).length;
...
function inferFunctionNameFromCode(code) {
    // function foo(
...
function findFunctionBlockByName(source, name) {
    const patterns = [
...
function findFirstFunctionBlock(source) {
    ...
    while ((m = rx.exec(source))) {
        ...
        const end = findMatchingBrace(source, startBrace);
        if (end > startBrace) {
            const functionName = (m[2] || m[3] || m[4]);
            const functionCode = source.slice(idx, end + 1);
            const surrounding = sliceSurrounding(source, idx, end + 1, 8);
...
function sliceSurrounding(source, start, end, lines) {
