...
describe('Error Delta Tracking', () => {
  let breaker: DualCircuitBreaker;

  beforeEach(() => {
    breaker = new DualCircuitBreaker(
      3,   // syntax_max_attempts  
...
function formatOutput(results) {
  return results.map(r => \`\${r.name}: \${r.value}\`);
    ...
    const buggyAnalysis = CodeErrorAnalyzer.analyzeCode(buggyCode);
    const fixedAnalysis = CodeErrorAnalyzer.analyzeCode(fixedCode);
    const comparison = CodeErrorAnalyzer.compareAnalyses(buggyAnalysis, fixedAnalysis);
  ...
  it('should allow extra attempts when errors are decreasing', () => {
    ...
    const decisions: Array<{ attempt: number; canAttempt: boolean; reason: string }> = [];
    ...
    iterations.forEach((iteration, index) => {
      ...
      const [canAttempt, reason] = breaker.can_attempt(ErrorType.LOGIC);
      const summary = breaker.get_state_summary();
  ...
  it('should stop when no improvement despite attempts', () => {
    ...
    stagnantIterations.forEach((iteration, index) => {
      ...
      const [canAttempt, reason] = breaker.can_attempt(ErrorType.SYNTAX);
      const summary = breaker.get_state_summary();
  ...
  it('should handle the user\'s specific example: missing comma fixing multiple errors', () => {
    ...
    breaker.record_attempt(
      ErrorType.SYNTAX,
      true, // The comma fix was successful
      afterCommaFix, // Errors remaining
      errorsResolvedByCommaFix // Errors this fix resolved
    );

    const [canContinue, reason] = breaker.can_attempt(ErrorType.SYNTAX);
    const summary = breaker.get_state_summary();
    ...
    const [stillCanContinue, nextReason] = breaker.can_attempt(ErrorType.SYNTAX);
