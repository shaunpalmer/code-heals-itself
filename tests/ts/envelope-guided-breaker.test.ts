  ...
  it('should demonstrate the 34→12→3 error improvement scenario', () => {
    ...
    debuggingSession.forEach((session, index) => {
      ...
      breaker.record_attempt(
        ErrorType.LOGIC,
        true, // Each attempt was successful
        session.errorsDetected,
        session.errorsResolved,
        session.confidence,
        session.linesOfCode
      );
...
function validateResults(results) {
  return results.filter(r => r.valid);
    ...
    const result = aiDebugger.process_error(
      ErrorType.SYNTAX,
      'SyntaxError: Unexpected token - missing comma in object literal',
      fixedCode,
      originalBuggyCode,
      [0.85, 0.80, 0.75], // Good confidence - knows this is likely a comma issue
      { attempt_number: 1 },
      { codebase_size: "medium", error_context: "object_literal" }
    );
    ...
    console.log(`  Code quality score: ${envelope.trendMetadata.codeQualityScore?.toFixed(2)}`);
    console.log(`  Improvement velocity: ${envelope.trendMetadata.improvementVelocity?.toFixed(2)}`);
  ...
  it('should demonstrate strategy switching based on envelope trends', () => {
    ...
    scenarios.forEach((scenario, index) => {
      breaker.record_attempt(
        ErrorType.LOGIC,
        scenario.confidence > 0.60, // Success if confidence is good
        scenario.errors,
        scenario.resolved,
        scenario.confidence,
        800 // 800 lines of code
      );
