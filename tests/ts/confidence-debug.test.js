...
describe('Confidence Calculation Debug', () => {
    it('should show actual confidence scores', () => {
        const aiDebugger = new ai_debugging_1.AIDebugger({
        ...
        testCases.forEach(testCase => {
            try {
                const result = aiDebugger.process_error(confidence_scoring_1.ErrorType.SYNTAX, `Test ${testCase.name}`, 'console.log("test");', 'console.log("broken"', testCase.logits);
                ...
                console.log(`${testCase.name} (${testCase.logits}): BLOCKED - ${error.message}`);
            }
        });
