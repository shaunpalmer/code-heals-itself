...
const ErrorSignature_1 = require("../../utils/typescript/ErrorSignature");
describe("ErrorSignature", () => {
    describe("normalize method", () => {
        it("should normalize basic JavaScript errors consistently", () => {
            ...
            expect(ErrorSignature_1.ErrorSignature.normalize(refError)).toBe("ReferenceError:x is not defined");
            expect(ErrorSignature_1.ErrorSignature.normalize(typeError)).toBe("TypeError:Cannot read property of undefined");
            expect(ErrorSignature_1.ErrorSignature.normalize(syntaxError)).toBe("SyntaxError:Unexpected token");
        });
        it("should remove file paths and line numbers", () => {
            const error = new Error("Something went wrong at file.js:10:5");
            const normalized = ErrorSignature_1.ErrorSignature.normalize(error);
        ...
        it("should handle non-Error objects", () => {
            ...
            expect(ErrorSignature_1.ErrorSignature.normalize(stringError)).toBe("String:Simple string error");
            expect(ErrorSignature_1.ErrorSignature.normalize(objectError)).toBe("Object:Object error");
        });
    });
    describe("create method", () => {
        it("should create detailed error signature", () => {
            const error = new ReferenceError("variable not found");
            const signature = ErrorSignature_1.ErrorSignature.create(error);
    ...
    describe("areSame method", () => {
        it("should identify identical error signatures", () => {
            ...
            expect(ErrorSignature_1.ErrorSignature.areSame(error1, error2)).toBe(true);
            expect(ErrorSignature_1.ErrorSignature.areSame(error1, error3)).toBe(false);
        });
    });
});
describe("ErrorTracker", () => {
    let tracker;
    beforeEach(() => {
        tracker = new ErrorSignature_1.ErrorTracker();
    });
