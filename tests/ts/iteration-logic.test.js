...
function calculateTotal(items) {
  ...
  return total;
        ...
        for (let i = 1; i <= 8; i++) { // More attempts to test circuit breaker behavior
            ...
            const beforeCanAttempt = breaker.can_attempt(confidence_scoring_1.ErrorType.LOGIC);
            ...
            const beforeState = aiDebugger.breaker.get_state_summary();
            const beforeMemory = aiDebugger.memory.buffer.length;
            const beforeCascade = aiDebugger.cascade.error_chain.length;
            console.log(`Before - Circuit: ${beforeState.state}, Memory: ${beforeMemory}, Cascade: ${beforeCascade}`);
            const result = aiDebugger.process_error(confidence_scoring_1.ErrorType.LOGIC, `Iteration ${i}: Array index error`, patchedCode, testCode, [0.4, 0.3, 0.3], // logits
            { previous_attempts: i - 1 });
            // Get state after processing
            const afterState = aiDebugger.breaker.get_state_summary();
            const afterMemory = aiDebugger.memory.buffer.length;
            const afterCascade = aiDebugger.cascade.error_chain.length;
    ...
    it('should test memory-based learning and strategy adaptation', async () => {
        ...
        const memory = aiDebugger.memory;
        ...
        const result = aiDebugger.process_error(confidence_scoring_1.ErrorType.LOGIC, testError, patchedCode, testCode, [0.7, 0.6, 0.7], {});
    ...
    it('should test circuit breaker progression and state transitions', async () => {
        ...
        for (let i = 1; i <= 8; i++) {
            const beforeState = aiDebugger.breaker.get_state_summary().state;
            const canAttempt = aiDebugger.breaker.can_attempt(confidence_scoring_1.ErrorType.LOGIC)[0];
            if (!canAttempt) {
            ...
            const result = aiDebugger.process_error(confidence_scoring_1.ErrorType.LOGIC, `Circuit test ${i}: Property access error`, patchedCode, testCode, [0.3, 0.2, 0.4], {});
            const afterState = aiDebugger.breaker.get_state_summary().state;
    ...
    it('should test cascade error detection and stopping conditions', async () => {
        ...
        for (let i = 1; i <= 6; i++) {
            ...
            const result = aiDebugger.process_error(confidence_scoring_1.ErrorType.LOGIC, repeatingError, // Same error repeatedly
            patchedCode, testCode, [0.4, 0.3, 0.3], {});
    ...
    it('should validate RETRY vs ROLLBACK decision logic', async () => {
        ...
        const scenarios = [
            { error: 'Syntax error: missing semicolon', type: confidence_scoring_1.ErrorType.SYNTAX, shouldRetry: true },
            { error: 'Logic error: infinite loop', type: confidence_scoring_1.ErrorType.LOGIC, shouldRetry: true },
            { error: 'Critical: memory exhausted', type: confidence_scoring_1.ErrorType.RUNTIME, shouldRetry: false }
        ];
        for (let i = 0; i < scenarios.length; i++) {
            ...
            const result = aiDebugger.process_error(scenario.type, scenario.error, "fixed code", "buggy code", [0.5, 0.5, 0.5], {});
            const breaker = aiDebugger.breaker;
